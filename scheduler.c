...
int sched_next_RR(Process *proc, int procnum){
    int nextid = -1;
    if(running == -1){
        nextid = (prerunning+1)%procnum;
        while(proc[nextid].pid == -1 || proc[nextid].remain == 0){
            nextid = (nextid+1)%procnum;
            if(nextid == (prerunning+1)%procnum){
                nextid = -1;
                break;
            }
        }
    }
    else if((time-last_cs)%500 == 0){
        int findnext = (running+1)%procnum;
        while(proc[findnext].pid == -1 || proc[findnext].remain == 0)
            findnext = (findnext+1)%procnum;
        nextid = findnext;
    }
    else
        nextid = running;
